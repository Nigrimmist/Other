...
function ssga.new(ua, domain, options) 
	...
	local function create_cookie() 
		...
		local time = ngx.now()
		local url = data['utmr']
		local fullUrl = data['utmr']
		local filename = ngx.var.uri
		...
		local cookie = '__utma='  .. rand_id .. '.' .. random .. '.' .. time .. '.' .. time .. '.' .. time .. '.2;+'
			...
			cookie = cookie .. '__utmz=' .. rand_id .. '.' .. time .. '.2.2.utmccn=' .. url .. '|utmcsr=' .. data['utmr'] .. '|utmcmd=' .. filename ..';+'
...
function track(options)
	...
	-- the UA part with MO
	local ssga = ssga.new(options.ua, options.domain, options) 
